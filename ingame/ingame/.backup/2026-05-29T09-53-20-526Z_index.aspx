		...
		<script type="text/javascript">
			...
			function renderMainBet(stageKey) {
				...
				g_mainBetSwiper = new Swiper(".gamemain-bet", {
					...
					centeredSlides:true, slideToClickedSlide:true, loop: stage && stage.items.length >= 3,
			...
			function openBetPage(stageKey, itemIdx) {
				...
				if (isGroup) {
					...
					g_betPredSwiper = new Swiper("#betPredictBox .betpage__betbox-bet", {
						...
						centeredSlides: true, slideToClickedSlide: true, loop: canLoop,
						navigation: {
			...
			function initCountdown() {
				if (g_dtCurrentStageBetEnd && g_dtCurrentStageBetEnd > new Date()) {
					if (typeof $.fn.countdown === 'function') {
						...
						// 備用計算（例如本地/預覽模式下未載入 countdown 插件時）
						var diffMs = g_dtCurrentStageBetEnd - new Date();
						var days = Math.floor(diffMs / (1000 * 60 * 60 * 24));
						var hours = Math.floor((diffMs % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
						var mins = Math.floor((diffMs % (1000 * 60 * 60)) / (1000 * 60));
						$("#countdownDisplay").html(days + '天 ' + hours + '時 ' + mins + '分');
					}
				} else {
